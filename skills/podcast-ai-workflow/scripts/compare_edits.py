...
def 解析指定路径(value, 期目录: Path):
    ...
    if path.is_absolute():
        return path
    for candidate in (期目录 / path, Path.cwd() / path):
        if candidate.exists():
            return candidate
    return 期目录 / path
...
def 写跨期复盘(期名列表, 表现数据路径=None, 输出路径=None):
    表现数据 = 读表现数据(表现数据路径)
    结果 = []
    for name in 期名列表:
        目录 = Path(name)
    ...
    report = Path(输出路径).expanduser() if 输出路径 else Path.cwd() / "跨期剪辑复盘.md"
...
def main():
    ...
    ap.add_argument("--输出", dest="output", help="跨期复盘报告路径；默认写到当前工作目录")
    ap.add_argument("-h", "--help", action="help", help="看用法")
    args = ap.parse_args()

    global 普通词表, 正则词表
    try:
        普通词表, 正则词表 = load_glossary(Path.cwd(), args.glossary)
    ...
    if args.汇总:
        ...
        try:
            写跨期复盘(args.期, args.表现数据, args.output)
