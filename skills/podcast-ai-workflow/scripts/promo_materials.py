...
def main():
    ...
    if args.final_transcript:
        指定稿 = Path(args.final_transcript).expanduser()
        if not 指定稿.is_absolute():
            单集候选 = 期目录 / 指定稿
            指定稿 = 单集候选 if 单集候选.exists() else Path.cwd() / 指定稿
        稿 = [指定稿.resolve()]
        if "原始" in 稿[0].name or "试跑" in 稿[0].name:
            sys.exit(f"❌ 宣发只能读取剪辑后的完整成片稿，不能使用：{稿[0].name}")
    else:
    ...
    if not 稿[0].is_file():
        sys.exit(f"❌ 找不到成片转写稿：{稿[0]}")
    if len(稿) > 1:
        names = "\n".join(f"   - {p.name}" for p in 稿)
        sys.exit("❌ 找到多份可能的成片稿，请用 --成片稿 明确指定：\n" + names)
    ...
    输出目录 = Path(args.output_dir).expanduser() if args.output_dir else 期目录
    if not 输出目录.is_absolute():
        输出目录 = Path.cwd() / 输出目录
    输出目录.mkdir(parents=True, exist_ok=True)
    sn = 输出目录 / f"{期目录.name}_ShowNotes素材.md"
    sn.write_text("\n".join(L) + "\n", encoding="utf-8")
