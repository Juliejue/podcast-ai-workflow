...
规则：
  ...
  - 复制 config.example.json → config.json，填写节目资料
  - 复制 glossary.example.json → glossary.json，增补常见误听词
"""
...
GLOSSARY_EXAMPLE_PATH = PROJECT_DIR / "glossary.example.json"
CONFIG_PATH = PROJECT_DIR / "config.json"
CONFIG_EXAMPLE_PATH = PROJECT_DIR / "config.example.json"


def load_config():
    """优先读私有 config.json；没有时使用可公开的示例配置。"""
...
def load_glossary():
    ...
    path = GLOSSARY_PATH if GLOSSARY_PATH.exists() else GLOSSARY_EXAMPLE_PATH
    if not path.exists():
...
def main():
    ...
    if not audio.is_absolute():
        audio = (PROJECT_DIR / audio).resolve()
    if not audio.exists():
        sys.exit(f"❌ 找不到音频：{audio}")

    episode = detect_episode(audio)
    outdir = audio.parent
    plain, regex = load_glossary()
    config = load_config()
    ...
    print("   发现新的听错词，加到 glossary.json 里，下次自动修。")
